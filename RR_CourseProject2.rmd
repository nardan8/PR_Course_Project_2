...
**Total sum of TOP-10**
```{r}
total_sum_health_dmg_top_10 <- sum(hd_data[1:10,"HEALTHDMG"])
```
...
ggplot(hd_data, aes(x = reorder(EVTYPE, -HEALTHDMG), y = HEALTHDMG)) +
        ...
        labs(title = "TOP 10 SEVERE WEATHER EVENTS", x = "WEATHER EVENTS", y = "FATALITIES + INJURIES")
```
...
**Percentage of the TOP-10 amount in the total amount**
```{r}
percentage_of_top_10_econ_dmg <- 
        sum(econdmg_data[1:10,"ECONDMG"]) / sum(econdmg_data[,"ECONDMG"]) * 100
```
...
ggplot(econdmg_data_to_plot, aes(x = reorder(EVTYPE, -ECONDMG), y = ECONDMG/1000000)) +
        geom_bar(stat = "identity", color = "black", fill = "steelblue4") +
